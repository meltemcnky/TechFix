...
export function OverflowMenu({children,label}:{children:ReactNode;label?:string}){
  ...
  return <div className="relative" ref={ref}><button type="button" aria-label={label??t('common.actions')} className="icon-button text-xl font-bold" onClick={()=>setOpen(x=>!x)}>⋮</button>{open?<div className="absolute right-0 z-30 mt-2 min-w-48 rounded-xl border border-slate-200 bg-white p-1 shadow-xl" onClick={()=>setOpen(false)}>{children}</div>:null}</div>;
}

export function MenuButton({children,danger=false,onClick}:{children:ReactNode;danger?:boolean;onClick:()=>void}){return <button type="button" className={`block w-full rounded-lg px-3 py-2 text-left text-sm ${danger?'text-red-700 hover:bg-red-50':'text-slate-700 hover:bg-slate-100'}`} onClick={onClick}>{children}</button>}
