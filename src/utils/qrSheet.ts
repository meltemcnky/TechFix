...
export async function createQrSheet(url:string,pin:string,labels:{title:string;help:string;warning:string}){
  ...
  context.fillStyle='#0f2344';context.textAlign='center';context.font='bold 42px sans-serif';context.fillText(labels.title,450,70);
  const image=new Image();image.src=qr;await image.decode();context.drawImage(image,90,100,720,720);
  context.font='24px sans-serif';context.fillStyle='#475569';context.fillText(labels.help,450,875);
  context.font='bold 58px monospace';context.fillStyle='#0f2344';context.fillText(pin,450,955);
  context.font='18px sans-serif';context.fillStyle='#64748b';context.fillText(labels.warning,450,1020);
